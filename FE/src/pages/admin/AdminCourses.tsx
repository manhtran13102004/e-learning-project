...
import { Plus, Pencil, Trash2, Eye, ChevronLeft, ChevronRight, Search, Layers } from "lucide-react"
...
export function AdminCourses() {
  ...
  function toggleSelectOne(courseId: number, checked: boolean) {
    setSelectedIds((prev) => (checked ? [...prev, courseId] : prev.filter((id) => id !== courseId)))
  }

  async function handleBulkDelete() {
    if (selectedIds.length === 0) return
    if (!confirm(`Xóa ${selectedIds.length} khóa học đã chọn?`)) return
    setIsBulkDeleting(true)
    setError(null)
    try {
      await adminService.deleteCoursesBulk(selectedIds)
      loadCourses()
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {selectedIds.length > 0 && (
            <Button variant="destructive" disabled={isBulkDeleting} onClick={handleBulkDelete}>
              <Trash2 className="mr-2 h-4 w-4" />
              {isBulkDeleting ? "Đang xóa..." : `Xóa đã chọn (${selectedIds.length})`}
            </Button>
          )}
      ...
      <div className="grid gap-1">
        ...
        <div className="flex flex-wrap gap-3 rounded-md border p-3">
          {categories.length === 0 && (
            <span className="text-sm text-muted-foreground">Chưa có danh mục nào</span>
          )}
          {categories.map((category) => {
            const checked = categoryFilterIds.includes(category.id)
            return (
              <label
                key={category.id}
                className="flex items-center gap-2 text-sm"
                htmlFor={`course-category-filter-${category.id}`}
              >
                <Checkbox
                  id={`course-category-filter-${category.id}`}
                  checked={checked}
                  onCheckedChange={(next) =>
                    setCategoryFilterIds((prev) =>
                      next ? [...prev, category.id] : prev.filter((id) => id !== category.id)
                    )
                  }
                />
            ...
            {courses.map((course) => (
              ...
              >
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <Checkbox
                    aria-label={`Chọn ${course.name}`}
                    checked={selectedIds.includes(course.id)}
                    onCheckedChange={(checked) => toggleSelectOne(course.id, checked)}
                  />
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                  <Button
                    ...
                    aria-label={`Quản lý nội dung ${course.name}`}
                  >
        ...
        <DialogContent className="max-w-2xl">
          ...
          {viewingCourse && (
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {viewingCourse.categoryName && (
                  <Badge variant="outline">{viewingCourse.categoryName}</Badge>
                )}
          ...
          <DialogFooter>
            <Button
              ...
              variant="outline"
              onClick={() => {
                if (viewingCourse) navigate(`/admin/courses/${viewingCourse.id}/content`)
              }}
            >
